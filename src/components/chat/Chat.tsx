...
export default function Chat() {
    ...
    // Performance optimization: useCallback prevents redefining `play` on every render.
    // This is necessary because `Chat` re-renders frequently due to `messages` state updates
    // (from events like 'tts-finished'), and stable references prevent child component re-renders.
    ...
    // Performance optimization: useCallback ensures `submit_pinyin` reference remains stable.
    // This prevents `InputArea` (which uses `onSendMessage={submit_pinyin}`) from unnecessarily
    // re-rendering when `messages` updates.
